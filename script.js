...
function displayTrees(trees) {
  trees.forEach((tree) => {
    ...
    card.className = `card bg-white shadow-sm border-b-2 ${tree.price > 500 ? "border-red-500" : "border-yellow-500"}`;
    card.innerHTML = `<figure>
      ...
      <div class="card-body">
        <h2 class="card-title cursor-pointer hover:text-[#0077FF]" onclick="openTreeModal(${tree.id})">${tree.name}</h2>
        <p class="line-clamp-2 text-[15px] text-gray-600">
          ${tree.description}
        </p>
        <div class="badge badge-info badge-outline text-[13px]">${tree.category}</div>
        <div class="flex justify-between items-center">
          <h2 class="font-bold text-xl ${tree.price > 500 ? "text-red-600" : "text-yellow-600"}">$${tree.price}</h2>
          </div>
